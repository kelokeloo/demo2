...
> 适用于本 Java 学习项目。每个知识点对应一个包，用 `Main` 作为唯一演示入口。

## 总则

1. 项目按**知识点**组织代码，而不是按层（controller/service/entity）。
2. 每个知识点是一个**包**，包路径 = 目录路径（`src` 为源码根目录）。
3. 每个知识点包内有一个 `Main.java`（含 `public static void main`），作为该知识点的唯一入口。
4. 只放真正被多个主题复用的类到 `common/`，避免重复维护。
...
### 新增知识点流程
1. 在 `src/topics/` 下新建 `<topic>/`，包名 `topics.<topic>`。
